...
function seed_cspv4_emaillist_database_add_subscriber(){
        ...
        if(isset($insert_result) && $insert_result != false){
            ...
            if($emaillist == 'database' && !empty($database_notifications)){
                // MOI
                /*$text = 'Gracias por visitar nuestra página web. En estos momentos estamos trabajando para cumplir tus sueños. Visítanos cualquiera de nuestras tiendas:'."\n\n";
                $text.= '- Centro Comercial Real Plaza Salaverry, Jesús María (Lima)  - al costado del BBVA Continental.'."\n";
                $text.= '- Motorplaza del Mall Aventura, Bellavista (Callao)'."\n\n\n";
                $text.= 'O llámanos al 715-6270 ó 715-6271'."\n\n\n";
                wp_mail($email,'Neolima Suscripción',$text);*/
                // END MOI
