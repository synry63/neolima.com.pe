...
class WPDDL_scripts_manager
{
	...
	private function __initialize_styles()
	{
		...
		$this->styles['layouts-settings-admin-css'] = new WPDDL_style(  'layouts-settings-admin-css', WPDDL_RES_RELPATH . '/css/ddl-settings.css', WPDDL_VERSION );
	...
	private function __initialize_scripts()
	{
		...
		$this->scripts['toolset-utils'] = new WPDDL_script('toolset-utils', WPDDL_TOOLSET_COMMON_RELPATH . "/utility/js/utils.js", array('jquery', 'underscore', 'backbone'), '1.0', true);
		...
		if( isset( $_GET['page'] ) && 'dd_layouts_edit' == $_GET['page'] )
		{
			...
			$this->scripts['icl_editor-script'] = new WPDDL_script('icl_editor-script',
				WPDDL_TOOLSET_COMMON_RELPATH . '/visual-editor/res/js/icl_editor_addon_plugin.js',
				array('views-codemirror-script'));
		...
		if( isset( $_GET['in-iframe-for-layout']) && $_GET['in-iframe-for-layout'] == 1 &&
			...
			$this->scripts['ddl-layouts-views-support'] = new WPDDL_script('ddl-layouts-views-support', WPDDL_RES_RELPATH . '/js/dd-layouts-views-support.js', array('jquery', 'suggest'), WPDDL_VERSION);
		}
		...
		if (isset( $_GET['in-iframe-for-layout']) &&
			...
			if ($post->post_type == CRED_FORMS_CUSTOM_POST_NAME) {
				$this->scripts['ddl-layouts-cred-support'] = new WPDDL_script('ddl-layouts-cred-support', WPDDL_RES_RELPATH . '/js/dd-layouts-cred-support.js', array('jquery'), WPDDL_VERSION);
			}
		}
        ...
        if (isset( $_GET['in-iframe-for-layout']) &&
            ...
            if ($post->post_type == CRED_USER_FORMS_CUSTOM_POST_NAME) {
                $this->scripts['ddl-layouts-cred-user-support'] = new WPDDL_script('ddl-layouts-cred-user-support', WPDDL_RES_RELPATH . '/js/dd-layouts-cred-user-support.js', array('jquery'), WPDDL_VERSION);
            }
        }
