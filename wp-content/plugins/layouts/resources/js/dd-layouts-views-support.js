...
DDLayout.layouts_views_support = function($)
{
    ...
    self.init = function( ) {
		...
		var data = window.parent.DDLayout.views_in_iframe.fetch_extra_controls('views-layouts-div');
		var controls = data.controls;
		
		//$('.wpv-settings-complete-output.js-wpv-settings-content').append(controls);
		$(controls).insertAfter('.wpv-title-section .wpv-settings-title-and-desc');
		
		$('#views-layouts-div .js-ddl-tag-name').val(data.tag);
		$('#views-layouts-div .js-edit-css-id').val(data.id);
		$('#views-layouts-div .js-edit-css-class').val(data.css);
		$('#views-layouts-div #ddl-default-edit-cell-name').val(data.name);
		
		// Fix widths for form inputs to fit Views with.
		$('#views-layouts-div input').css('width' , '440px');
