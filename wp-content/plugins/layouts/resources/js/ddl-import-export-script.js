                                        ...
                                        if ( $is_zip === 0 ){
                                            ...
                                            if ( $show_log ){
                                                jQuery.each($import_files_array, function(index, val) {
                                                    $('.import-layouts-messages').append('<p class="ddl-import-file-'+index+'">'+ val +' <span><i class="icon icon-spinner icon-spin"></i></span>' + '</p>');
                                                });
                                            }
                                            $('html,body').animate({
            ...
            $.ajax({
                ...
                error: function(x, t, m) {
                        if(t==="timeout") {
                            $skipped_layouts++;
                            if ( $show_log === true ){
                                $('.ddl-import-file-'+$current_file).find('span').html(' <i class="icon-remove"></i>');
                            }
                            $current_file ++;
                            temp_res = {
                                file : $current_file,
                ...
                success: function(response){
                    if ( (typeof(response) !== 'undefined') ) {
                        ...
                        self.count_imported_files( temp_res );
                        $('.ddl-import-file-'+$current_file).find('span').html(' <i class="icon-ok "></i>');
                        $current_file ++;
                        self.continue_import( temp_res );
                    }
                    else{
